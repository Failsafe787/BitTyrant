...
public class ConfigSectionTransfer implements UISWTConfigSection {
	...
	public Composite configSectionCreate(final Composite parent) {
		...
		if (userMode > 0) {
			...
			////
			final String enableMaxConnKey = "Enable.AutoMaxConnections";
			boolean autoMaxConnectionsEnabled = COConfigurationManager.getBooleanParameter(enableMaxConnKey, true);
			final BooleanParameter enableAutoMaxConnections = new BooleanParameter(cSection,
					enableMaxConnKey, autoMaxConnectionsEnabled, "ConfigView.label.enable_auto_max_connections");
			gridData = new GridData();
			gridData.horizontalSpan = 2;
			...
			//useless?
			gridData = new GridData();
			Label maxPeersPerTorrent = new Label(cSection, SWT.NULL);
			maxPeersPerTorrent.setLayoutData(gridData);
			Messages.setLanguageText(maxPeersPerTorrent, "ConfigView.label.max_peers_per_torrent");
			//useless?
			gridData = new GridData();
			gridData.widthHint = 35;
			...
			//useless?
			gridData = new GridData();
			Label maxPeersTotal = new Label(cSection, SWT.NULL);
			maxPeersTotal.setLayoutData(gridData);
			Messages.setLanguageText(maxPeersTotal, "ConfigView.label.max_peers_total");
			//cSection.setEnabled(false);
			...
			//enable/disable controls for max num connections
			final Control[] proxy_controls = new Control[] { maxPeersTotal,
					maxPeersPerTorrent, paramMaxClients.getControl(), paramMaxClientsTotal.getControl() };

			IAdditionalActionPerformer autoMax = new GenericActionPerformer(
					new Control[] {}) {
			...
			enableAutoMaxConnections.setAdditionalActionPerformer(new GenericActionPerformer(
					new Control[] {}) {
				public void performAction() {
					COConfigurationManager.setParameter(enableMaxConnKey, enableAutoMaxConnections.isSelected());
				}
			});

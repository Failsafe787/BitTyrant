	...
	implements PEPeerTransport
{
  ...
  public int[] getIncomingRequestedPieceNumbers() {
	...
	/**
	 * Makes this transport sortable by download speed from the remote peer
	 */
	public int compareTo(PEPeerTransport other) {
		//return new Long(getStats().getDataReceiveRate()).compareTo(other.getStats().getDataReceiveRate());
		return new Double(getTyrantScore()).compareTo(other.getTyrantScore());
	}


	private TyrantStats tStats = null;
	public synchronized TyrantStats getTyrantStats() {
		if(tStats == null)
			tStats = new TyrantStats(this);
		return tStats;
	}

//	public void updatePeerBandwidthAllocation(final int maxUpBytesPerSecond, final int maxDownBytesPerSecond){
//		//override whatever settings existed before
//		NetworkManager.getSingleton().stopTransferProcessing(connection);
//		LimitedRateGroup up = new LimitedRateGroup(){
//
//			public int getRateLimitBytesPerSecond() {
//				return maxUpBytesPerSecond;
//			}
//			
//		};
//		LimitedRateGroup down = new LimitedRateGroup(){
//
//			public int getRateLimitBytesPerSecond() {
//				// TODO Auto-generated method stub
//				return maxDownBytesPerSecond;
//			}
//			
//		};
//		
//		//reregister the connection with the new limits
//		connection.startMessageProcessing(up, down);
//	}
	...
	/**
	 * Assign a score to this peer based on the amount we expect them to recipricate
	 * assuming we send enough data to fall into their active set.
	 * @return an altruism score, lower is more favorable for us
	 */
	public double getTyrantScore(){
		if(connection == null)
			//bail out
			return Double.MAX_VALUE;
		
		if(this.peerSupportsMessageType(AZMessage.ID_AZ_BTYRANT_SELFISH)){
			//they told us that they won't be altruistic
			return 1.0;
		}
		...
		if(getStats().getTotalDataBytesSent() < (50*1024)){ 
			//we haven't sent many pieces, so we can't judge if they'll recipricate
			//we know they haven't identified themselves as greedy, so they might be a sucker
			return 0.99;
		}
		
		//leave with the overall ratio of what we've sent versus what we've received.  Lower is better
		return ((double)getStats().getTotalDataBytesSent())/getStats().getTotalProtocolBytesReceived();
	}
	...
	public boolean isSelfishTyrantPeer() {
		String id = getClient();
		// TODO this is a hack... clients really should send msgs instead of relying on peerid
		if(id == null)
			return false;
		return id.endsWith(TyrantStats.TYRANT_GUI_TEXT_STRING);
	}
}

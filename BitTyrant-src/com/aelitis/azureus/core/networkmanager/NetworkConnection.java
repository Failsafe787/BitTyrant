...
/**
 * Represents a managed network connection, over which messages can be sent and
 * received.
 */
public interface NetworkConnection {
	...
	/**
	 * Connect this connection's transport, i.e. establish the network
	 * connection. If this connection is already established (from an incoming
	 * connection for example), then this provides a mechanism to register the
	 * connection listener, in which case connectSuccess() will be called
	 * immediately.
	 * 
	 * @param listener
	...
	/**
	 * Find out how many bytes are available for use by this particular network
	 * connection. Note: can be null if there is not limit on this connection
	 * 
	 * @return a value representing the most bytes allowed to be sent on this
	 *         connection
	 */
	 ...
	 * 
	 * @param limiter
	 *            a per-connection limit on upload
	 */
	public void setBandwidth(int limit);

	/**
	 * A weight for selecting this connection for upload frequency
	 */
	public void setWeight(double connectionWeight);
	...
	public interface ConnectionListener {
		/**
		 * The connection establishment process has started, i.e. the connection
		 * is actively being attempted.
		 */
		...
		/**
		 * The connection attempt failed. NOTE: Called only during initial
		 * connect attempt.
		 * 
		 * @param failure_msg
		 *            failure reason
		...
		/**
		 * Handle exception thrown by this connection. NOTE: Can be called at
		 * any time during connection lifetime.
		 * 
		 * @param error
		 *            exception
		 */

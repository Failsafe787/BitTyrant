				...
				new TimerEventPerformer()
				{
						...
						TimerEvent	event )
					{
						try{
							...
							//rebuildIPBloomFilter( false );
							DHTLog.log("dhtdb stats: " + getStats().getKeyCount() + " keys");
							Iterator kit = getKeys();
							while(kit.hasNext()){
								Object key = kit.next();
								System.out.println("key: " + ByteFormatter.encodeString(((HashWrapper)key).getHash()));
								Iterator vit = ((DHTDBMapping)stored_values.get(key)).getValues();
								while(vit.hasNext()){
									Object v = vit.next();
									DHTDBValue val = (DHTDBValue) v;
									System.out.println("val string: "+val.getString() + " val values " + ByteFormatter.encodeString(val.getValue()));
								}
							}
						}finally{

...
public class SeedingUnchoker implements Unchoker {
  ...
  public SeedingUnchoker() {
		if (Logger.isEnabled())
			Logger.log(new LogEvent(this, logging, "using seeding unchoker"));
  }
  ...
  public void calculateUnchokes( int max_to_unchoke, ArrayList all_peers, boolean force_refresh ) {
    //make sure bandwidth limits aren't preventing any seeindg
	  for(int i = 0; i < all_peers.size(); i++){
		  PEPeerTransportProtocol peer = (PEPeerTransportProtocol)all_peers.get( i );
	    	peer.getNetworkConnection().setBandwidth(Integer.MAX_VALUE);
	    	peer.getNetworkConnection().setWeight(1000);
	}
